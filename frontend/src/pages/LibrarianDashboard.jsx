...
const LibrarianDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [complaintsRes, finesRes] = await Promise.all([
        api.get('/librarian/complaints'),
        api.get('/librarian/fines')
      ]);
      setComplaints(complaintsRes.data.complaints);
      setFines(finesRes.data.fines);
      try {
        const fb = await api.get('/librarian/feedback');
  ...
  const handleStatusUpdate = async (complaintId, status, adminResponse = '') => {
    try {
      await api.put(`/librarian/complaints/${complaintId}`, { status, adminResponse });
      fetchData();
    } catch (error) {
  ...
  const handleFineSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/librarian/fines', fineForm);
  ...
  const handleFineDelete = async (id) => {
    ...
    try {
      await api.delete(`/librarian/fines/${id}`);
      fetchData();
    } catch (error) {
  ...
  const handleFeedbackDelete = async (id) => {
    ...
    try {
      await api.delete(`/librarian/feedback/${id}`);
      fetchData();
    } catch (error) {
