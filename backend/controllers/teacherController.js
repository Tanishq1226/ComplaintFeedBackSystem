...
export const getComplaints = async (req, res) => {
  try {
    const complaints = await Complaint.find({ department: 'academics' })
...
export const getFines = async (req, res) => {
  try {
    const fines = await Fine.find({ department: 'academics' })
      .populate('student', 'name email studentId')
...
export const getFeedbacks = async (req, res) => {
  try {
    const feedbacks = await Feedback.find({ department: 'academics' })
      .populate('student', 'name email studentId')
      .sort({ createdAt: -1 });
